...
do
    ...
    DENOM="uflix"                           # Coin denominator is uatom ("microoatom"). 1 atom = 1000000 uatom.
    MINIMUM_DELEGATION_AMOUNT="20000000"    # Only perform delegations above this amount of uatom. Default: 2atom.
    RESERVATION_AMOUNT="10000000"          # Keep this amount of uatom in account. Default: 10uflix.
    VALIDATOR="omniflixvaloper12ke3jzkyhcrdkf9q4eh4edrgueygr4lsq4wmr3"        # Default is Validator Network.
    ...
    CHAIN_ID="omniflixhub-1"                                     # Current chain id. Empty means auto-detect.
    NODE="https://omniflix.kingnodes.com:443"  # Either run a local full node or choose one you trust.
    GAS_PRICES="0.02uflix"                         # Gas prices to pay for transaction.
    GAS_ADJUSTMENT="1.30"                           # Adjustment for estimated gas
    GAS_FLAGS="--gas auto --gas-prices ${GAS_PRICES} --gas-adjustment ${GAS_ADJUSTMENT}"
    GAIABIN=omniflixhubd
    ##############################################################################################################################################################
    ...
    then
        ...
        echo ${PASSPHRASE} | $GAIABIN tx distribution withdraw-rewards ${VALIDATOR} --yes --from ${KEY} --sequence ${ACCOUNT_SEQUENCE} --chain-id ${CHAIN_ID} --node ${NODE} ${GAS_FLAGS} 
        ACCOUNT_SEQUENCE=$((ACCOUNT_SEQUENCE + 1))
    fi

    printf "Delegating... "
    echo ${PASSPHRASE} | $GAIABIN tx staking delegate ${VALIDATOR} ${DELEGATION_AMOUNT}${DENOM} --yes --from ${KEY} --sequence ${ACCOUNT_SEQUENCE} --chain-id ${CHAIN_ID} --node ${NODE} ${GAS_FLAGS}
